...
void Mesh::GenerateNormals() {
	...
	for (unsigned int i = 0; i < numVertices; ++i) {
		...
		normal.Normalise();
		normals[i] = normal; //Above way doesn't work because I changed asciimesh file and am using squares.
...
Mesh*	Mesh::GenerateSquarePatch() {
	...
	m -> BufferData();//FunctioncontainingalloftheVAOandVBOsetup

	return m;
}

Mesh*	Mesh::GenerateLaser() {
	Mesh * m = new Mesh();
	m->numVertices = 4;//Numberofvertices
	m->type = GL_PATCHES;//TheOpenGLrendertype
	...
	m->colours[3] = Vector4(1.0f, 1.0f, 0.0f, 0.0f);
